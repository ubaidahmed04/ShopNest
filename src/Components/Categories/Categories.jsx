import React from 'react'
import { Card ,Button } from 'react-bootstrap'
import category1 from '../../Images/category1.png'
import './Category.css'

function Categories() {
  return (
    <div className='main-card '>
      <div className='leaf-img'><img src="data:image/png;base64," alt="" /></div>
      <div className='heading position-relative '>NEW ARRIVALS</div>
        <div className='row card-body '>
            <div className='col-4 col-md-6 col-sm-12  my-3  category-Card'>
         <Card className='card-box'>
      <Card.Img variant="top" className='category-img' src={category1} />
      <Card.Body>
        <Card.Title className='title'>Hoodies & Sweetshirt</Card.Title>
        <div className='d-flex justify-content-between flex-row'>
            <div>
        <Card.Text>
          
                <div className='explore'>Explore Now!</div>
        </Card.Text>
            </div>
            <div>
            <i class="fa-solid fa-arrow-right fa-xl"></i>
            </div>
        </div>
       
      </Card.Body>
    </Card>
            </div>
            <div className='col-6 col-md-6 col-sm-12 my-3  category-Card'>
         <Card className='card-box'>
      <Card.Img variant="top" className='category-img' src={category1} />
      <Card.Body>
        <Card.Title className='title'>Hoodies & Sweetshirt</Card.Title>
        <div className='d-flex justify-content-between'>
            <div>
        <Card.Text>
          
                <span className='explore'>Explore Now!</span>
        </Card.Text>
            </div>
            <div>
            <i class="fa-solid fa-arrow-right fa-xl"></i>
            </div>
        </div>
       
      </Card.Body>
    </Card>
            </div>
            <div className='col-4 col-md-6 col-sm-12 my-3 category-Card'>
         <Card className='card-box'>
      <Card.Img variant="top" className='category-img' src={category1} />
      <Card.Body>
        <Card.Title className='title'>Hoodies & Sweetshirt</Card.Title>
        <div className='d-flex justify-content-between'>
            <div>
        <Card.Text>
          
                <span className='explore'>Explore Now!</span>
        </Card.Text>
            </div>
            <div>
            <i class="fa-solid fa-arrow-right fa-xl"></i>
            </div>
        </div>
       
      </Card.Body>
    </Card>
            </div>
            <div className='col-4 col-md-4 col-sm-12 my-3 category-Card' >
         <Card  className='card-box'>
      <Card.Img variant="top" className='category-img' src={category1} />
      <Card.Body>
        <Card.Title className='title'>Hoodies & Sweetshirt</Card.Title>
        <div className='d-flex justify-content-between'>
            <div>
        <Card.Text>
          
                <span className='explore'>Explore Now!</span>
        </Card.Text>
            </div>
            <div>
            <i class="fa-solid fa-arrow-right fa-xl"></i>
            </div>
        </div>
       
      </Card.Body>
    </Card>
            </div>
        </div>
    </div>
  ) 
}

export default Categories